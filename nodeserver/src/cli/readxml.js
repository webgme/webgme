...
define([ "util/assert", "util/sax", "fs", "core/core", "core/tasync" ], function (ASSERT, SAX, FS, Core, TASYNC) {
	...
	function old (storage, filename, callback) {
		...
		var timerhandle = setInterval(function () {
			console.log("  at line " + parser._parser.line + " (" + total + " xml objects, " + idCount + " ids, " + unresolved.length + " idrefs)");
		}, 2000);
			...
			if (err) {
				...
				core.persist(tags[0].node, function (err) {
					if (err) {
						...
						console.log("Parsing done (" + total + " xml objects, " + idCount + " ids, " + unresolved.length + " idrefs)");
		...
		var resolveLoadByPath = function (path, callback) {
			core.loadByPath(root, path, callback);
		};

		var resolveAddPointer = function (source, name, targetPath, callback) {
			ASSERT(typeof name === "string" && typeof targetPath === "string" && callback instanceof Function);
		...
		stream.on("error", function (err) {
			exit(err.code === "ENOENT" ? "File not found: " + filename : "Unknown file error: " + JSON.stringify(err));
		});
