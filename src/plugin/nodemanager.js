...
function PluginNodeManager(webGMESessionId, project, mainLogger, gmeConfig) {
    var self = this,
        logger = mainLogger.fork('PluginNodeManager'),
    ...
    this.executePlugin = function (pluginName, pluginConfig, context, callback) {
        var plugin,
            pluginLogger = logger.fork('plugin:' + pluginName);
    ...
    this.loadContext = function (context) {
        ...
        Q.ninvoke(project, 'loadObject', context.commitHash)
            ...
            .then(function (activeSelection) {
                ...
                return self.loadNodesByPath(pluginContext.rootNode, metaIds);
            })
    ...
    this.loadNodesByPath = function (nodePaths, rootNode, returnNameMap) {
        var deferred = Q.defer(),
